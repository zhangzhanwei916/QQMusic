...
//  详情控制器, 负责歌曲信息的展示, 播放的业务逻辑(上一首, 下一首, 播放, 暂停等等), 歌词的展示
...
//        self.addChildViewController(tvc)
    ...
    // 负责更新歌曲信息的定时器(可以指定更新的时间)
    var timer: NSTimer?
    
    // 负责更新歌词的定时器(更新频率非常高, 大概一秒60次)
...
extension QQDetailVC {
    ...
    @IBAction func playOrPause(sender: UIButton) {
        ...
        // 此处的播放或者暂停, 是直接调用工具类的接口, 而不是在此处写具体的实现, 一定要注意封装
        if sender.selected {
